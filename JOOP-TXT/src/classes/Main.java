...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		System.out.println("Dseja 'Carregar' os arquivos Gravados ou Iniciar um 'Novo'? 1 - Carregar | 2 - Novo");
		int response = sc.nextInt();
		
		if (response == 1) {
			file.readFile(FILE, list);
		} else {
			file.createAndDeleteFile(FILE, response);
		}
		...
		while (confirm) {
			...
			default:
				...
				if (finish == 1) {
					System.out.println();
					System.out.println("Salvando dados armazenados !");
					file.writeToFile(list, FILE);
